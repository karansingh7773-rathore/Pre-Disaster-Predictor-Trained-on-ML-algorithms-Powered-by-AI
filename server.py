...
class MultiDisasterModelTrainer:
    ...
    def load_training_data(self, file_path='sample_training_data.csv'):
        """Load training data from a CSV file."""
        try:
            df = pd.read_csv(file_path)
            print(f"Successfully loaded training data from {file_path}")
            return df
    ...
    def train_disaster_models(self):
        """Train models for all disaster types using data from a CSV file."""
        disaster_types = ['flood', 'earthquake', 'landslide', 'cyclone', 'drought']
        
        # Load the dataset
        df = self.load_training_data()
        if df is None:
            print("Model training failed because the dataset could not be loaded.")
            return None

        for disaster_type in disaster_types:
            print(f"Training {disaster_type} prediction model...")
            
            target_column = f'{disaster_type.capitalize()}_Risk'
            if target_column not in df.columns:
                print(f"Warning: Target column '{target_column}' not found in the dataset. Skipping {disaster_type} model.")
                continue

            # Drop rows where the target is NaN
            df_disaster = df.dropna(subset=[target_column])

            X = df_disaster.drop(columns=[f'{dt.capitalize()}_Risk' for dt in disaster_types])
            y = df_disaster[target_column]
            
            # Ensure all feature columns are present, fill missing with 0 or a suitable default
            for col in self.numerical_features + self.categorical_features:
                if col not in X.columns:
                    X[col] = 0 # Or a more intelligent default
